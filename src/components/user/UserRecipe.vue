      ...
      <div class="d-flex flex-sm-row flex-column justify-content-between align-items-sm-center">
        ...
        <div>
          <button class="btn add-btn px-3 py-2 rounded-pill" @click="$router.push( '/new-recipe' )">
            <i class="fa-solid fa-circle-plus pe-2"></i>Add Recipe</button>
        </div>
      </div>
    </li>
    <li class="list-group-item">
      ...
      <div class="row">
        <!-- User Recipe Card -->
        <user-recipe-card v-for="recipe in recipes" :key="recipe.id" :recipe="recipe" :buttonName="['Delete', 'Edit']" @btnRemove="deleteRecipe(recipe.id)" @btnUpdate="updateRecipe(recipe.id)">
...
const updateRecipe = async (id) => {
  await store.dispatch("recipe/updateRecipe", id);
